...
def test_place_order_and_login_before_checkout(page: Page, session_user):
    # 1. Launch browser
    # 2. Navigate to url 'http://automationexercise.com'
    # 3. Verify that home page is visible successfully
    ...
    checkout_page.validate_delivery_address_is_correct(session_user)
    checkout_page.validate_billing_address_is_correct(session_user)

    # 12. Enter description in comment text area and click 'Place Order'
    checkout_page.add_order_comment(session_user)
    checkout_page.click_place_order()

    # 13. Enter payment details: Name on Card, Card Number, CVC, Expiration date
    payment_page = PaymentPage(page).wait_for_load()
    payment_page.fill_payment_form(session_user)
